...
async function fetchDexScreener(address: string): Promise<DexScreenerSummary> {
  try {
    ...
    // Choose the top pair by highest liquidity
...
export async function POST(req: Request) {
  try {
    ...
    // If it looks suspicious and also not a contract snippet or address, reject it
    ...
    if (isAddress(content)) {
      ...
      if (dexData?.chainId) {
        ...
        if (metaWrap?.json && metaWrap?.baseUrl) {
          ...
          // Try to resolve entry file from compilationTarget
          ...
          // Fallback: if entry missing, pick the largest source file
          if (!entrySource) {
      ...
      // 3) Holders distribution (Ethereum only via Ethplorer)
      ...
      // 4) Build context text for the model
